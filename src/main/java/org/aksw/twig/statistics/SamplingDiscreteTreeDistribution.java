...
public class SamplingDiscreteTreeDistribution<T> implements SamplingDiscreteDistribution<T> {
  ...
  /**
   * Whenever a discrete event is added via {@link #addDiscreteEvent(Object, double)} its chance
   * will be added to the aggregated chance of all events. The optimal aggregated chance is
   * obviously one. But since chances might be calculated inaccurate, events might sum up to aa
   * aggregated chance being truly greater than one. You can set a clearance for the aggregated
   * chance in constructor. The clearance will allow the aggregated chance to be less or equal to
   * {@code 1 + aggregatedChanceDelta}. For example if your aggregated chance is 1.001 and
   * {@code aggregatedChanceDelta} is 0.002 everything is fine.<br>
   * <br>
   *
   * Note that there will be small inaccuracies for random events since the event that lead to the
   * aggregated chance being truly greater than 1 will be slightly less probable than it should be.
   * It is not possible to have events that can't occur at all. An exception will be thrown if you
   * add an element and the aggregated chance is greater than one although the aggregated chance was
   * in clearance.<br>
   * <br>
   *
   * It is possible to have an aggregated chance truly smaller than one. If this is the case, then
   * the last added event will be slightly more probable than it should be. To be exact: Let
   * {@code e} be the last added event of the sample space with an aggregated chance {@code P} of
   * all events truly smaller than one. Let {@code p} be the chance of {@code e}. When sampling
   * events from the sample space {@code e's} chance will be {@code p' := p + (1 - P)}.
   */
  public final double aggregatedChanceDelta;

  /**
   * Creates a new instance setting class variables. {@link #aggregatedChanceDelta} will be set to
   * {@code 0.0}.
   */
  ...
  public void addDiscreteEvent(T event, double chance)
    ...
    if (aggregatedChance >= 1) {
      throw new IllegalStateException(
          "Aggregated chance was >= 1 - would lead into impossible event");
    }
