...
public class ExponentialLikeDistributionTest {
  ...
  public void sampleTest() {
    ...
    /*
     * ExponentialLikeDistribution d = new ExponentialLikeDistribution(0.01); int steps = 1000;
     * double sampleAverage = 0.0; for (int i = 0; i < steps; i++) { sampleAverage += (double)
     * d.sample() / (double) steps; } Assert.assertEquals(d.getNumericalMean(), sampleAverage, 5d);
     */
  }
  ...
  public void probabilityTest() {
    ...
    Assert.assertEquals(d.cumulativeProbability(x0, x1),
        d.cumulativeProbability(x1) - d.cumulativeProbability(x0), 0.00001);
  }
}
