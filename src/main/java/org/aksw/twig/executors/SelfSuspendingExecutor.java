...
/**
 * This class works like a {@link Executors#newFixedThreadPool(int)} with one difference.
 * {@link Callable} objects get pulled from a {@link SuspendSupplier} which will then be executed.
 * Results will be handed vice versa to the {@link SuspendSupplier}. The major advantage of this
 * class is that it will only pull and execute {@link Callable} objects from its supplier as long as
...
public class SelfSuspendingExecutor<T> {
   ...
   */
  public SelfSuspendingExecutor(final SuspendSupplier<T> suspendSupplier, final int nThreads) {
  ...
  /**
   * Starts callable processing. Method is non-blocking, i. e. termination of this method does not
   * mean every callable has been executed.
   */
  public void start() {
  ...
  private void execNext() {
    ...
    final Callable<T> callable = suspendSupplier.next();
    if (callable == null) {
