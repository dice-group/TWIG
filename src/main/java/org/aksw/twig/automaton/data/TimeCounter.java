...
public class TimeCounter implements Serializable {
  ...
  /**
   * Returns a matrix of hours and minutes with following semantics: {@code x := matrix[h][m]} means
   * that {@code x} timestamps have been counted between {@code h:m} o'clock (inclusive) and
   * {@code h:(m+1)} o'clock (exclusive).
   * 
  ...
  /**
   * Returns number of timestamps at given time. Equivalent to {@code matrix[hour][minute]} in
   * {@link #getTweetTimes()}.
   * 
   * @param hour Hour.
   * @param minute Minute.
  ...
  public void addModel(Model model) {
    model.listStatements().forEachRemaining(statement -> {
      if (statement.getPredicate().getLocalName()
        ...
        LocalDateTime time =
            LocalDateTime.from(TWIGModelWrapper.DATE_TIME_FORMATTER.parse(literal.getString()));
  ...
  /**
   * Creates a discrete distribution frequency measure by the timestamp counts as frequency
   * distribution.
   * 
   * @return Discrete distribution.
   */
  public SamplingDiscreteDistribution<LocalTime> getValueDistribution() {
    SamplingDiscreteTreeDistribution<LocalTime> distribution =
        new SamplingDiscreteTreeDistribution<>(DISTRIBUTION_CHANCE_DELTA);
